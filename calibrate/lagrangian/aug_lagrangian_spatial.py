...
class SpatialAugLagrangian(AugLagrangian):
    def __init__(
        self,
        num_classes: int = 10,
        margin: float = 10,
        ...
        self.lambd = self.lambd_min * torch.ones(1, requires_grad=False).cuda()
        # self.rho = self.rho_min
        # class-wise rho
        self.rho = self.rho_min * torch.ones(1, requires_grad=False).cuda()
        # for updating rho
    ...
    def reset_update_lambd(self, epoch):
        ...
        self.class_weights = torch.zeros(self.num_classes,device=self.lambd.device)
    ...
    def update_lambd(self, logits, targets, epoch):
        ...
        # print (self.class_weights.shape)
        for ii in range(self.num_classes):
            self.class_weights[ii] += torch.sum(targets == ii)
        
        self.grad_p_sum += grad_p.mean(dim=1).sum(dim=0)
        self.sample_num += grad_p.shape[0]
        self.curr_constraints += h.mean(dim=1).sum(dim=0)
