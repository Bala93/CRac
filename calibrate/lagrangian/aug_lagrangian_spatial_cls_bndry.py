...
class SpatialBndClsAugLagrangian(AugLagrangian):
    def __init__(
        self,
        num_classes: int = 10,
        margin: float = 10,
        ...
        self.lambd = self.lambd_min * torch.ones(self.num_classes * 2, requires_grad=False).cuda()
        # self.rho = self.rho_min
        # class-wise rho
        self.rho = self.rho_min * torch.ones(self.num_classes * 2, requires_grad=False).cuda()
        # for updating rho
    ...
    def get_constraints(self, logits, targets, dim: int = -1):
        ...
        constraints_easy = torch.abs(self.margin * rmask - logits) #* 
        # constraints_easy = F.relu(self.margin * rmask - logits) #* 
        constraints_diff = torch.abs(rmask - logits) #self.margin 
        # constraints_easy = F.relu(self.get_diff(logits) - self.margin)
        # constraints_diff = F.relu(self.margin * rmask - logits) #self.margin 
    ...
    def get(self, logits: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        reg_idx = reg_idx.movedim(1,-1)
        hcat = torch.cat([h * reg_idx, h * (1-reg_idx)], -1)
        # print (hcat.shape, self.lambd.shape, self.rho.shape)
        # print (hcat.shape, self.lambd.shape, self.rho.shape)
    ...
    def get_diff(self, inputs):
        ...
        max_values = max_values.values.unsqueeze(dim=1)
    ...
    def update_lambd(self, logits, targets, epoch):
        ...
        h = h.movedim(1, -1) ## to have same dimension as that of logit.
        reg_idx = reg_idx.movedim(1,-1)
        # print (torch.unique(reg_idx, return_counts=True))
        ...
        _, grad_p = self.penalty_func(hcat, self.lambd, self.rho)
