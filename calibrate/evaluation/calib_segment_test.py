...
def calib_metrics(preds, targets):
    ...
    tcece = metrics.ThreshClasswiseECELoss()(preds, targets).item()
    
    # briers = metrics.seg_brier_score(preds, targets).item()
    
    return np.round(ece,4), np.round(cece,4), np.round(aece,4), np.round(nll,4), np.round(tcece,4) #, np.round(briers,4)
...
class CalibSegmentEvaluator(DatasetEvaluator):
    ...
    def reset(self):
        ...
        self.total_aece = np.empty((0,1), dtype=np.float32)
        self.total_nll = np.empty((0,1), dtype=np.float32)
        self.total_tcece = np.empty((0,1), dtype=np.float32)
        self.total_briers = np.empty((0,1), dtype=np.float32)
    ...
    def update(self, pred: np.ndarray, target: np.ndarray, logits, labels, fpath):
        ...
        # print (dsc)
        briers = metrics.seg_brier_score(logits, labels, self.num_classes)
        ...
        ece, cece, aece, nll, tcece = calib_metrics(logits, labels)
        ...
        self.total_aece = np.vstack([self.total_aece, aece])
        self.total_nll = np.vstack([self.total_nll, nll])
        self.total_briers = np.vstack([self.total_briers, briers])
        self.total_tcece = np.vstack([self.total_tcece, tcece])
    ...
    def mean_score(self):
        ...
        maece = self.total_aece.mean()
        mnll = self.total_nll.mean()
        mbrier = self.total_briers.mean()
        mtcece = self.total_tcece.mean()
        
        return {"mdsc": mdsc, "mhd": mhd, "mece": mece, "mcece": mcece, "maece": maece, 
                "mnll": mnll, "mbrier": mbrier, "mtcece": mtcece}
    ...
    def calib_score(self, isprint=False, return_dataframe=True):
        ...
        aece = self.total_aece.mean()
        nll = self.total_nll.mean()
        briers = self.total_briers.mean()
        tcece = self.total_tcece.mean()

        columns = ["ece", "cece", "aece", "nll", "briers", "tcece"]
        table_data = [columns]
        table_data.append(
            [
                "{:.5f}".format(ece),
        ...
        if return_dataframe:
            ...
            data[columns[2]].append(aece)
            data[columns[3]].append(nll)
            data[columns[4]].append(briers)
            data[columns[5]].append(tcece)
    ...
    def save_csv(self,save_dir):
        ...
        with open(os.path.join(save_dir,'metrics.csv'),'w') as f:
            for fname, dsc, hd, ece, cece in zip(self.file_names, self.total_dsc, 
                                                 self.total_hd, self.total_ece, self.total_cece):
                f.write(','.join([fname] + [str(ii) for ii in dsc] + 
                                 [str(ii) for ii in hd] + [str(ece)] + [str(cece)] ) + '\n')