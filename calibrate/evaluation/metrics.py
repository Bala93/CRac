...
def seg_brier_score(logits, targets, num_classes):
    ...
    rmask = rmask.reshape(bs, num_classes, h, w)

    n, c, x, y = logits.shape
    targets = targets.squeeze()
    targets = targets.reshape(n * x * y)
    index = torch.nonzero(targets != 0).squeeze()
    logits = torch.einsum("ncxy->nxyc", logits)
    logits = logits.reshape(n * x * y, -1)
    rmask = torch.einsum("ncxy->nxyc", rmask)
    rmask = rmask.reshape(n * x * y, -1)
    logits = logits[index,:]
    rmask = rmask[index,:]
    
    softmaxes = F.softmax(logits, dim=1)
    brier = torch.mean(torch.sum((softmaxes - rmask) ** 2, dim=1))
